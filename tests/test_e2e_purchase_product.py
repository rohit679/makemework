...
def test_e2e_weather_shopper(base_url, browser, browser_version, os_version, os_name,\
    ...
    try:
        ...
        test_obj.log_result(result_flag,
                            positive="Landed on the %s page after clicking the buy button"\
                            %product_type, negative="Could not land on the %s page after \
                            clicking the buy button"%product_type, level="critical")
        #Add products
        ...
        test_obj.log_result(result_flag,
                            positive="Something wrong with the cart. \
                            The log messages above will have the details",
                            negative="Something wrong with the cart.\
                            The log messages above will have the details",
                            level="critical")

        #Go to the Pay with card
        result_flag = test_obj.go_to_pay()
        test_obj.log_result(result_flag,
                            positive="Automation is now on the payment form",
        ...
        print("Python says:%s"%repr(exception))
